...
// Events > cy.on()
// Stub a function > cy.stub()
// Click on a point > by cy.document()
...
describe('Session 6', function () {
    ...
    it.only('2-2 Alerts - Confirm - Dismiss', function () {
        cy.visit('https://the-internet.herokuapp.com/javascript_alerts')
        cy.contains('Click for JS Confirm').click()
        cy.on('window:confirm', function (message) {
            expect (message).eq('I am a JS Confirm')
    ...
    it('3- Alerts - Prompt', function () {
        ...
        cy.window().then(function ($win) {
            ...
            cy.stub($win, 'prompt').returns("AutomationCamp-cypress") //stub
        })
